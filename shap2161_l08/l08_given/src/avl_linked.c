...
static void avl_rebalance(avl_node **node) {
    int balance_factor = avl_balance(*node);
...
static BOOLEAN avl_remove_aux(avl_linked *source, avl_node **node, const data_ptr key, data_ptr item) {
    ...
    if (*node != NULL) {
        ...
        if (cmp < 0) {
            r = avl_remove_aux(source, &((*node)->left), key, item);
        } else if (cmp > 0) {
            r = avl_remove_aux(source, &((*node)->right), key, item);
        } else {
            avl_node *temp = *node;
            if ((*node)->left == NULL) {
                *node = (*node)->right;
                ...
                *node = (*node)->left;
                ...
                r = avl_remove_aux(source, &((*node)->right), successor->item, item);
            }
